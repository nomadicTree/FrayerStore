...
def get_words_for_course(course_id: int) -> List[Word]:
    """Return Word objects filtered to versions linked to topics in a course."""
    db = get_db()
    q = """
        SELECT DISTINCT w.id AS word_id, w.word AS word_name
        FROM WordVersionContexts wvc
        ...
        ORDER BY w.word COLLATE NOCASE
    """
    rows = db.execute(q, (course_id,)).fetchall()
    ...
    for row in rows:
        ...
        for version in word.versions:
            matching_topics = [
                topic for topic in version.topics if topic.course.course_id == course_id
            ]
            if matching_topics:
                version.topics = matching_topics
                filtered_versions.append(version)
